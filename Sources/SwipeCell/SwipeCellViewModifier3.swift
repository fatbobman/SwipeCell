...
extension SwipeCellModifier{
    func getGesture() -> _EndedGesture<_ChangedGesture<DragGesture>>{
        ...
        return DragGesture(minimumDistance: editMode?.wrappedValue == .active ? 10000 : 1, coordinateSpace: .local)
            .onChanged{value in
                ...
                case .showCell:
                    ...
                    if width > 0 {
                        if leftSlot?.slotStyle == .destructive {
                            ...
                            if width > leftdestructiveWidth && (feedStatus == .none || feedStatus == .feedAgain)  {
                    ...
                    if width < 0 {
                        if rightSlot?.slotStyle == .destructive {
                            if width < -rightdestructiveWidth && (feedStatus == .none || feedStatus == .feedAgain)  {
                                successFeedBack(swipeCellStyle.vibrationForDestructive)
                                feedStatus = .feedOnce
                            }
                ...
                case .showLeftSlot:
                    if leftSlot?.slotStyle == .destructive {
                        if width > 0 {
                            if width + leftSlotWidth > leftdestructiveWidth && (feedStatus == .none || feedStatus == .feedAgain)  {
                                successFeedBack(swipeCellStyle.vibrationForDestructive)
                                feedStatus = .feedOnce
                            }
                            ...
                            if width + leftSlotWidth > leftdestructiveWidth {
                                withAnimation(.easeInOut){
                                    offset = leftdestructiveWidth +  (width + leftSlotWidth - leftdestructiveWidth)/5
                                    lastButtonOffset(position: .left, slot: leftSlot)
                                    lastButtonOffset(position: .right,slot:rightSlot)
                                }
                            }
                ...
                case .showRightSlot:
                    if rightSlot?.slotStyle == .destructive {
                        if width < 0 {
                            if -width + rightSlotWidth > rightdestructiveWidth && (feedStatus == .none || feedStatus == .feedAgain)  {
                                successFeedBack(swipeCellStyle.vibrationForDestructive)
                                feedStatus = .feedOnce
                            }
                            if -width + rightSlotWidth <= rightdestructiveWidth && feedStatus == .feedOnce {
                                successFeedBack(swipeCellStyle.vibrationForDestructive)
                                feedStatus = .feedAgain
                            }
                            //超过阈值,则移动减速
                ...
                if feedStatus == .feedAgain && (swipeCellStyle.destructiveWidth - abs(offset)) > swipeCellStyle.dismissWidth {
                    resetStatus()
                    return
                }
                ...
                case .showCell:
                    ...
                    if  leftSlot?.slotStyle == .normal {
                        ...
                        if (cellPosition == .left || cellPosition == .both) && width >= swipeCellStyle.appearWidth && width <= leftdestructiveWidth{
                            withAnimation(leftSlot?.appearAnimation){
                                offset = leftSlotWidth
                                lastButtonOffset(position: .left, slot: leftSlot)
                                lastButtonOffset(position: .right,slot:rightSlot)
                        ...
                        if (cellPosition == .left || cellPosition == .both) && width > leftdestructiveWidth{
                            resetStatus()
                    ...
                    if  rightSlot?.slotStyle == .normal {
                        ...
                        if (cellPosition == .right || cellPosition == .both) && width <= swipeCellStyle.appearWidth && width >= -rightdestructiveWidth{
                            withAnimation(rightSlot?.appearAnimation){
                                offset = -rightSlotWidth
                                lastButtonOffset(position: .left, slot: leftSlot)
                                lastButtonOffset(position: .right,slot:rightSlot)
                        ...
                        if (cellPosition == .right || cellPosition == .both) && width < -rightdestructiveWidth{
                            resetStatus()
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5){
                ...
                case .showLeftSlot:
                    if (abs(width) < swipeCellStyle.dismissWidth && (width + leftSlotWidth) <= leftdestructiveWidth) {
                        withAnimation(leftSlot?.appearAnimation){
                            offset = leftSlotWidth
                            lastButtonOffset(position: .left, slot: leftSlot)
                            lastButtonOffset(position: .right,slot:rightSlot)
                ...
                case .showRightSlot:
                    if (abs(width) < swipeCellStyle.dismissWidth && (-width + rightSlotWidth) <= leftdestructiveWidth) {
                        withAnimation(rightSlot?.appearAnimation){
                            offset = -rightSlotWidth
                            lastButtonOffset(position: .left, slot: leftSlot)
                            lastButtonOffset(position: .right,slot:rightSlot)
